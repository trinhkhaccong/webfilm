...
export default async function getServerSideProps(
    ...
    const result_title = await excuteQueryOb({
        query: 'SELECT name FROM tb_theloai WHERE ?',
        values: [ob],
    });

    const result_total = await excuteQueryOb({
        query: 'SELECT COUNT(*) as total_count FROM tb_theloai_intro WHERE id_theloai IN (SELECT id FROM tb_theloai WHERE ?)',
        values: [ob],
    });
    const result = await excuteQuery({
        query: 'SELECT link, link_background,name FROM `tb_filmintro` WHERE link IN (SELECT root_link as link FROM tb_theloai_intro WHERE id_theloai IN (SELECT id FROM tb_theloai WHERE ?)) ORDER BY timestamp DESC LIMIT '+LIMIT +' OFFSET '+OFFSET,
        values: [ob],
    });
